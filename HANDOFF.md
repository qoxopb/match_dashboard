...
- **git 주의**: 레포가 private (`github.com/qoxopb/match_dashboard`). 공용PC에서 pull 안 되면 레포를 public으로 전환하면 해결됨 (config/credentials는 gitignore 처리됨)
...
### git pull 공용PC 인증 문제
레포가 private이라 공용PC 윈도우 자격증명이 다른 계정이면 pull 실패. 해결: GitHub에서 레포를 public으로 전환 (config/credentials는 gitignore 처리됨, 코드에 시크릿 없음).

---

**현재 버전: v1.4.3 (2026-05-07)**
